...
function loadBorrowersListings(){
  console.log("Borrowers Listings Call");
  suserId = getCookie("sUserId");
  sprimaryType = getCookie("sUserType");
  saccessToken = getCookie("saccessToken");
...
$('.panFileUpload').fileupload({
    dataType: 'json',
    forceIframeTransport: true,
    done: function (e, data) {
      console.log(e, data);
        $.each(data.result.files, function (index, file) {
            $('body').text(file.name).appendTo(document.body);
        });
    }
});

$('.aadharFileUpload').fileupload({
    dataType: 'json',
    forceIframeTransport: true,
    done: function (e, data) {
      console.log(e, data);
...
function setKYCvars(){
  //checkCookie();
  suserId = getCookie("sUserId");
  var uploadurl ="http://ec2-52-66-245-153.ap-south-1.compute.amazonaws.com:8080/"+suserId+"/upload/kyc"
  $(".panFileUpload, .aadharFileUpload, .passportFileUpload").attr("data-url", uploadurl);
