      ...
      success: function(data,textStatus,xhr){
       console.log(data);
        ...
        if(data.amountDisbursed == null){
         ...
         //$(".closedLoansValue").html(data.noOfClosedLoans);
         ...
         $(".data-lender-commitement").html(data.commitmentAmount-data.amountDisbursed)
...
function loadborrowerDashboardData(){
    ...
    $.ajax({
      ...
      success: function(data,textStatus,xhr){
       ...
       $(".totalTransactionFee").html(data.totalTransactionFee);
       $(".amountDisbursed").html(data.amountDisbursed);
       $(".loanAmountPending").html(data.commitmentAmount-data.outstandingAmount);
...
function readBankStatement(input) {
 ...
 if (input.files && input.files[0]) {
   ...
   reader.onload = function(e) {
     ...
     $('.bsUpload .file-upload-image').attr('src', e.target.result);
...
var getBANKSTATEMENT = "http://ec2-52-66-245-153.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/"+suserId+"/download/BANKSTATEMENT";
              ...
              $.ajax({
                ...
                success: function(data,textStatus,xhr){
                 ...
                 if(data.downloadUrl != ""){
                    ...
                    $(".bsImage").attr('src',data.downloadUrl);
                 }
              },
                error: function(xhr,textStatus,errorThrown){
                  console.log('Error Something');
    ...
    $.ajax({
      ...
      success: function(data,textStatus,xhr){
       ...
       if(data.loanRequestAmount > loanMaxAmount ){
          ...
          console.log(2);
          var ansforconsole2 = data.loanRequestAmount-requesterdBalanec;
          if(ansforconsole2 < 0){
    ...
    $.ajax({
     ...
     if(data.firstName == null && data.lastName == null && data.fatherName == null && data.dob == null && data.address == null && data.nationality == null&& data.gender == null&& data.maritalStatus == null){
      ...
      //alert("poda");
        ...
        $("#profile-submit-btn, #firstname, #lastname, #fathername, .date, #nationality,#address, .genderInfo, .maritalStatusInfo, .udisplaysec").hide();
...
function loadAgreedloans(){
 ...
 var postData = JSON.stringify(postData);
console.log(postData);
 var getStatUrl = "http://ec2-52-66-245-153.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/"+userId+"/loan/"+primaryType+"/search";
...
function esignDone(loanRequestIDFromElement){
  ...
  sprimaryType = getCookie("sUserType");
  saccessToken = getCookie("saccessToken");
  var esignAgree = "http://ec2-52-66-245-153.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/"+suserId+"/loan/"+sprimaryType+"/"+loanRequestIDFromElement+"/uploadAgreement";
...
function loadActiveLoans(){
  ...
  var postData = {
    ...
    var postData = JSON.stringify(postData);
    console.log(postData);
     var actOnLoan = "http://ec2-52-66-245-153.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/"+userId+"/loan/"+primaryType+"/search";
        $.ajax({
          url:actOnLoan,
          type:"POST",
          data:postData,
