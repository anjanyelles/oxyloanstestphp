    ...
    */
	...
	$("#loan-submit-btn").click(function () {
		...
		if (enteredloanRequestAmount == "") {
			...
			if (primaryType == "LENDER") {
				$(".loanRequestAmountError").html(
					"As per RBI guidelines lender can lend only 10 Lakhs only."
				);
				$(".loanRequestAmountError").show();
				isValid = false;
			}
			if (primaryType == "BORROWER") {
				$(".loanRequestAmountError").html(
					"As per RBI guidelines borrower can borrow only 10 Lakhs only."
				);
				$(".loanRequestAmountError").show();
				isValid = false;
			}
...
function loadDashboardData() {
	...
	$.ajax({
		...
		success: function (data, textStatus, xhr) {
			...
			var intPaid = Math.ceil(data.interestPaid);
			intPaid = Number(intPaid.toFixed(1)); // 6.7

			console.log("intPaid is " + intPaid);
			$(".interestPaid").html(intPaid);
			...
			var principalReceived = Math.ceil(data.principalReceived);
			principalReceived = Number(principalReceived.toFixed(1)); // 6.7

			$(".principalReceived").html(principalReceived);
...
function getUserDetails() {
	...
	if (userisIn == "local") {
		...
		var personalDetailsUrl =
			"http://ec2-13-232-202-148.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/personal/" +
			suserId +
			"";
	} else {
		// https://fintech.oxyloans.com/oxyloans/
...
function loadviewLoanoffer() {
	...
	$.ajax({
		...
		success: function (data, textStatus, xhr) {
			...
			$(".rateOfInterest option[value=" + data.rateOfInterest + "]").attr(
				"selected",
				"selected"
			);
			$("#expectedDate").val(data.loanRequestedDate);
			$(".durationValue option[value=" + data.duration + "]").attr(
				"selected",
				"selected"
			);
...
function loadprofessionalDetails() {
	...
	if (userisIn == "local") {
		//http://ec2-13-232-202-148.ap-south-1.compute.amazonaws.com:8080/oxyloans/
		var getStatUrl =
			"http://ec2-13-232-202-148.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/professional/" +
			id +
			"";
	} else {
		// https://fintech.oxyloans.com/oxyloans/
...
function loadfinancialDetails() {
	...
	if (userisIn == "local") {
		//http://ec2-13-232-202-148.ap-south-1.compute.amazonaws.com:8080/oxyloans/
		var getStatUrl =
			"http://ec2-13-232-202-148.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/getfinancialDetails/" +
			id +
			"";
	} else {
		// https://fintech.oxyloans.com/oxyloans/
	...
	$.ajax({
		...
		success: function (data, textStatus, xhr) {
			...
			$(".displaycreditcardsrepaymentamount").html(
				data.creditCardsRepaymentAmount
			);
...
$(document).ready(function () {
	setTimeout(function () {
		$(".sendActivationLink").click(function () {
			...
			transedEmail = userEmail.replace(
				transedEmail.slice(4, -4),
				"*".repeat(transedEmailLength - 2)
			); // console.log(postData);
			//alert(transedEmail);
			/// return false;
			...
			$.ajax({
				...
				success: function (data) {
					...
					$("#emailMessage #inner-message").html(
						"We've sent an activation link to your email. i.e " + transedEmail
					);
					console.log(data);
				},
				statusCode: {
